...
export const Reviews: MessageFns<Reviews> = {
  encode(message: Reviews, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    globalThis.Object.entries(message.reviews).forEach(([key, value]: [string, Review]) => {
  ...
  fromJSON(object: any): Reviews {
    return {
      reviews: isObject(object.reviews)
        ? (globalThis.Object.entries(object.reviews) as [string, any][]).reduce(
          (acc: { [key: number]: Review }, [key, value]: [string, any]) => {
  ...
  toJSON(message: Reviews): unknown {
    const obj: any = {};
    if (message.reviews) {
      const entries = globalThis.Object.entries(message.reviews) as [string, Review][];
      if (entries.length > 0) {
        obj.reviews = {};
        entries.forEach(([k, v]) => {
  ...
  fromPartial<I extends Exact<DeepPartial<Reviews>, I>>(object: I): Reviews {
    const message = createBaseReviews();
    message.reviews = (globalThis.Object.entries(object.reviews ?? {}) as [string, Review][]).reduce(
      (acc: { [key: number]: Review }, [key, value]: [string, Review]) => {
        if (value !== undefined) {
...
export const Reviews_ReviewsEntry: MessageFns<Reviews_ReviewsEntry> = {
  ...
  fromJSON(object: any): Reviews_ReviewsEntry {
    return {
      ...
      value: isSet(object.value) ? Review.fromJSON(object.value) : undefined,
    };
  },
