...
export const MapDefinitions: MessageFns<MapDefinitions> = {
  encode(message: MapDefinitions, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    globalThis.Object.entries(message.maps).forEach(([key, value]: [string, MapDefinition]) => {
    ...
    return {
      maps: isObject(object.maps)
        ? (globalThis.Object.entries(object.maps) as [string, any][]).reduce(
          (acc: { [key: number]: MapDefinition }, [key, value]: [string, any]) => {
  ...
  toJSON(message: MapDefinitions): unknown {
    const obj: any = {};
    if (message.maps) {
      const entries = globalThis.Object.entries(message.maps) as [string, MapDefinition][];
      if (entries.length > 0) {
        obj.maps = {};
        entries.forEach(([k, v]) => {
  ...
  fromPartial<I extends Exact<DeepPartial<MapDefinitions>, I>>(object: I): MapDefinitions {
    const message = createBaseMapDefinitions();
    message.maps = (globalThis.Object.entries(object.maps ?? {}) as [string, MapDefinition][]).reduce(
      (acc: { [key: number]: MapDefinition }, [key, value]: [string, MapDefinition]) => {
        if (value !== undefined) {
...
export const MapDefinitions_MapsEntry: MessageFns<MapDefinitions_MapsEntry> = {
  ...
  fromJSON(object: any): MapDefinitions_MapsEntry {
    return {
      ...
      value: isSet(object.value) ? MapDefinition.fromJSON(object.value) : undefined,
    };
  },
