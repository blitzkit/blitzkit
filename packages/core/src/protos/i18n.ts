...
export const I18nString: MessageFns<I18nString> = {
  encode(message: I18nString, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    globalThis.Object.entries(message.locales).forEach(([key, value]: [string, string]) => {
  ...
  toJSON(message: I18nString): unknown {
    const obj: any = {};
    if (message.locales) {
      const entries = globalThis.Object.entries(message.locales) as [string, string][];
      if (entries.length > 0) {
        obj.locales = {};
        entries.forEach(([k, v]) => {
          obj.locales[k] = v;
  ...
  fromPartial<I extends Exact<DeepPartial<I18nString>, I>>(object: I): I18nString {
    const message = createBaseI18nString();
    message.locales = (globalThis.Object.entries(object.locales ?? {}) as [string, string][]).reduce(
