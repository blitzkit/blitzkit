...
export async function GET({ props }: APIContext<{ path: string }>) {
  ...
  if (await vfs.resolve(`Data/${props.path}.packed.webp`)) {
    ...
    if (sizes) {
      buffer = await extractPackedIcon(consumablesTexture, sizes);
    } else {
      buffer = await consumablesTexture.trim({ threshold: 80 }).toBuffer();
    }
  } else {
    ...
    const sizes = (await vfs.text(`Data/${props.path}.txt`))
      ...
      .split(" ")
      .map(Number);
    buffer = await consumablesTexture
      .clone()
      .extract({
        left: sizes[0],
        top: sizes[1],
